...
export class KuCoinPublicConnector {
    ...
    private async getBulletPublic(): Promise<BulletPublicResponse['data']> {
        ...
        try {
            const response = await axios.post<BulletPublicResponse>(`${this.apiBaseUrl}${endpoint}`);
            logger.info(`Obtained bullet public token: ${response.data.data.token}`);
            logger.info(`Instance Servers: ${JSON.stringify(response.data.data.instanceServers, null, 2)}`); // Detailed logging
            return response.data.data;
        } catch (error: any) {
            logger.error(`Error obtaining bullet public: ${error.response?.data || error.message}`);
            throw error;
        }
    }
    ...
    public async connect() {
        ...
        try {
            ...
            const server = bullet.instanceServers.find(server => server.protocol === 'websocket') || bullet.instanceServers[0];
            ...
            this.wsUrl = `${server.endpoint}?token=${bullet.token}&connectId=${Date.now()}&env=prod`;
            ...
            this.connection.on('close', (code: number, reason: string) => {
                logger.warn(`Public WebSocket connection closed. Code: ${code}, Reason: ${reason}`);
                if (this.keepRunning) {
    ...
    private attemptReconnect() {
        ...
        const delay = Math.min(this.reconnectInterval * 2 ** this.reconnectAttempts, 60000); // Max 60 seconds
        logger.info(`Reconnecting in ${delay / 1000} seconds... (Attempt ${this.reconnectAttempts + 1}/${this.maxReconnectAttempts})`);
        setTimeout(() => {
